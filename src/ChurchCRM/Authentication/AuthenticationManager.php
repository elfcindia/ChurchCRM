...
class AuthenticationManager
{
    ...
    public static function ensureAuthentication(): void
    {
        ...
        try {
            ...
            if (!$result->isAuthenticated) {
                ...
                if ($safeUri !== '' && !self::isDisallowedPostLoginRedirect($safeUri)) {
    ...
    private static function validateRedirectPath(?string $url): ?string
    {
        ...
        if ($validated === '' || self::isDisallowedPostLoginRedirect($validated)) {
            return null;
        }
    ...
    /**
     * Prevent post-login redirects to static assets or non-page endpoints.
     * These URLs can be captured when the browser requests JS/CSS/images before
     * the user is authenticated (leading to confusing redirects after login).
     */
    private static function isDisallowedPostLoginRedirect(string $relativeUrl): bool
    {
        $path = ltrim($relativeUrl, '/');
        $pathLower = strtolower($path);

        // Block known non-page prefixes (relative to root path)
        foreach (['skin/', 'api/', 'session/'] as $prefix) {
            if (str_starts_with($pathLower, $prefix)) {
                return true;
            }
        }

        // Block common static file extensions
        return (bool) preg_match('/\.(?:js|css|map|png|jpe?g|gif|svg|ico|webp|woff2?|ttf|eot)$/i', $pathLower);
    }
