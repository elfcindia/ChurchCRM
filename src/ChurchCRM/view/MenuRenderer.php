...
class MenuRenderer
{
    ...
    /**
     * Render menu icon: Font Awesome classes on $item, or img:RelativePath under web root.
     */
    private static function renderNavIcon(MenuItem $menuItem): void
    {
        $icon = $menuItem->getIcon();
        if ($icon !== '' && str_starts_with($icon, 'img:')) {
            $rel = ltrim(substr($icon, 4), '/');
            $src = SystemURLs::getRootPath() . '/' . $rel;
            ?>
                    <span class="nav-link-icon d-inline-block align-middle">
                        <img src="<?= InputUtils::escapeAttribute($src) ?>" alt="" width="20" height="20" class="crm-menu-icon-img" loading="lazy">
                    </span>
            <?php
        ...
        ?>
                    <span class="nav-link-icon d-inline-block">
                        <i class="fa <?= InputUtils::escapeHTML($icon) ?>"></i>
                    </span>
        <?php
    }
            ...
            <div class="d-flex align-items-center">
                <a href="<?= InputUtils::escapeAttribute($menuItem->getURI()) ?>"
                   ...
                   class="nav-link<?= $isActive ? ' active' : '' ?> flex-fill">
                    <?php self::renderNavIcon($menuItem); ?>
