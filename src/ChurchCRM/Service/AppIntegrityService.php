...
class AppIntegrityService
{
    ...
    private static $OrphanedFilesDetails;

    // Both checks walk the entire application tree (4000+ files including vendor/),
    // which is expensive on slow filesystem I/O (network storage, Docker bind mounts).
    // Cache results across requests instead of recomputing on every page load.
    private const CACHE_TTL_SECONDS = 900;

    private static function readThroughCache(string $cacheFileName, callable $compute): array
    {
        $cacheFile = AppIntegrityService::resolveDocumentRoot() . '/logs/' . $cacheFileName;

        if (is_file($cacheFile)) {
            $cached = json_decode((string) @file_get_contents($cacheFile), true);
            if (is_array($cached) && isset($cached['computedAt'], $cached['result'])
                && (time() - (int) $cached['computedAt']) < AppIntegrityService::CACHE_TTL_SECONDS
            ) {
    ...
    {
        // In-memory cache avoids re-scanning within the same request when multiple
        // callers (status + message + files) all need the data. A short-lived file
        // cache (see readThroughCache()) avoids re-hashing 4000+ files on every
        // single admin page load — the scan still runs at least every 15 minutes,
        // so tampering is caught promptly without making every request pay for it.
        if (AppIntegrityService::$IntegrityCheckDetails !== null) {
            return AppIntegrityService::$IntegrityCheckDetails;
        }

        $verificationResult = AppIntegrityService::readThroughCache(
            'integrity-check-cache.json',
            fn (): array => AppIntegrityService::verifyApplicationIntegrity()
        );
    ...
    {
        if (AppIntegrityService::$OrphanedFilesDetails !== null) {
            return AppIntegrityService::$OrphanedFilesDetails;
        }

        AppIntegrityService::$OrphanedFilesDetails = AppIntegrityService::readThroughCache(
            'orphaned-files-cache.json',
            fn (): array => AppIntegrityService::scanForOrphanedFiles()
        );

        return AppIntegrityService::$OrphanedFilesDetails;
    }

    private static function scanForOrphanedFiles(): array
    {
        $logger = LoggerUtils::getAppLogger();
        $documentRoot = AppIntegrityService::resolveDocumentRoot();
        $signatureFile = $documentRoot . '/admin/data/signatures.json';
    ...
    public static function deleteOrphanedFiles(): array
    {
        ...
        // Invalidate the cache so the next check reflects the files just removed.
        AppIntegrityService::$OrphanedFilesDetails = null;
        @unlink(AppIntegrityService::resolveDocumentRoot() . '/logs/orphaned-files-cache.json');
