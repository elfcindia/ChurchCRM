...
<?php if (empty($bOmitAuthFooterBranding)): ?>
...
<?php endif; ?>
  ...
  <script nonce="<?= SystemURLs::getCSPNonce() ?>">
    // Logged-out pages do not load Footer.js; ensure #crmGlobalLoading is always removed.
    // (locale-loader also removes it — this covers stale cached locale-loader.min.js.)
    (function () {
      function hideCrmLoading() {
        document.getElementById("crmGlobalLoading")?.remove();
      }
      window.addEventListener("CRM.localesReady", hideCrmLoading, { once: true });
      setTimeout(hideCrmLoading, 12000);
    })();
  </script>
