...
$app->group('/api/demo', function (RouteCollectorProxy $group): void {
    ...
    $group->post('/load', function (Request $request, Response $response): Response {
        ...
        try {
            $logger->info('Admin demo data import started', ['includeEvents' => $includeEvents, 'includeSundaySchool' => $includeSundaySchool]);
            $demoService = new DemoDataService();
            $result = $demoService->importDemoData($includeEvents, $includeSundaySchool);
